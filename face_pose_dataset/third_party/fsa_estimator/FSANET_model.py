...
class BaseFSANet(object):
    ...
    def ssr_G_model_build(self, img_inputs):
        ...
        feat_s3_pre = AveragePooling2D((2, 2))(
            feat_s3_pre
    ...
    def ssr_feat_S_model_build(self, m_dim):
        ...
        if self.is_varS_model:
            ...
            feat_preS = Conv2D(1, (1, 1), padding="same", activation="sigmoid")(
                input_preS
            )

        feat_preS = Reshape((-1, self.map_xy_size * self.map_xy_size))(feat_preS)

        SR_matrix = Dense(
            m_dim * (self.map_xy_size * self.map_xy_size * 3), activation="sigmoid",
        )(feat_preS)
        SR_matrix = Reshape((m_dim, (self.map_xy_size * self.map_xy_size * 3)))(
            SR_matrix
        )
    ...
    def ssr_S_model_build(self, num_primcaps, m_dim):
        ...
        SL_matrix = Dense(int(num_primcaps / 3) * m_dim, activation="sigmoid")(
            feat_pre_concat
        )

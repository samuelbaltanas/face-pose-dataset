...
def reconstruct_vertex(param, whitening=True, dense=False, transform=True):
    ...
    if whitening:
        if len(param) == 62:
            param = param * par.param_std + par.param_mean
        else:
    ...
    if dense:
        vertex = (
            p
            @ (par.u + par.w_shp @ alpha_shp + par.w_exp @ alpha_exp).reshape(
                3, -1, order="F"
            )
            + offset
        )
        ...
        if transform:
            ...
            vertex[1, :] = par.std_size + 1 - vertex[1, :]
    else:
        """For 68 pts"""
        vertex = (
            p
            @ (
                par.u_base + par.w_shp_base @ alpha_shp + par.w_exp_base @ alpha_exp
            ).reshape(3, -1, order="F")
            + offset
        )
...
class DDFADataset(data.Dataset):
    def __init__(self, root, filelists, param_fp, transform=None, **kargs):
        ...
        self.params = torch.from_numpy(_load_cpu(param_fp))
